...
# Download and extract the latest ip_arp_parser release
...
if [ "$1" = "--help" ] || [ "$1" = "-h" ]; then
    echo "IP ARP Parser Download Script"
    echo "============================="
    echo
...
FILENAME="ip_arp_parser-${VERSION}-${PLATFORM}.${EXT}"
DOWNLOAD_URL="https://github.com/${REPO}/releases/download/${VERSION}/${FILENAME}"
CHECKSUM_URL="${DOWNLOAD_URL}.sha256"

echo "🚀 Downloading ip_arp_parser ${VERSION} for ${PLATFORM}..."
echo "📦 File: ${FILENAME}"
...
echo "📁 Contents:"
ls -la ip_arp_parser* README.md ip-arp-sample.json 2>/dev/null || ls -la

echo
echo "🎉 Ready to use! Try running:"
if [[ "$PLATFORM" == *"windows"* ]]; then
    echo "   ./ip_arp_parser.exe --help"
    echo "   ./ip_arp_parser.exe -input show-ip-arp.txt"
else
    echo "   ./ip_arp_parser --help"
    echo "   ./ip_arp_parser -input show-ip-arp.txt"
fi

echo
echo "📚 Usage examples:"
echo "   # Parse from input file to JSON output"
echo "   ./ip_arp_parser -input show-ip-arp.txt -output arp-results.json"
echo
echo "   # Get data directly from switch using commands.json"
echo "   ./ip_arp_parser -commands commands.json -output arp-results.json"
echo
